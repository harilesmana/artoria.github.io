...
//const SCROLL = ['1st Weapon', '2nd Weapon', '3rd Weapon', 'Scroll Type', '1st Skill', '2nd Skill', '3rd Skill']
...
const SLANG_NAMES = {'0': 'Random', '1': 'Fire', '2': 'Thunder', '3': 'Wind', '4': 'Water', '5': 'Earth', '6': 'Clone', '7': 'Shuriken', '8': 'Kunai'};
const WEAPON_TYPES = {'-1': 'Any', '1': '1H Sword', '2': '2H Sword', '3': 'Bow', '4': 'Bowgun', '5': 'Katana', '6': 'Halberd', '7': 'Staff', '8': 'Magic Device', '9': 'Knuckles'};
...
const findCombination = function (combination, byWeapons = false) {
    let matches = [];
    for (const scroll of SCROLL_COMBINATIONS) {
        const scroll_components = byWeapons?scroll.slice(0, 3):scroll.slice(4);
        let count = 0;
        for (const component of combination) {
            if (scroll_components.search(component) != -1) {
                count += 1;
            }
        }

        let cs_randoms = combination.split('0').length-1;
        let ss_randoms = scroll_components.split('0').length-1;
...
const scrollTypeList = function (scroll) {
    ...
    if (skills.split('0').length - 1 < 2) {
        ...
        skills.split('').forEach(function (value) {
            if (value > 0 && types.indexOf(SCROLL_TYPES[value]) == -1) {
...
const skillList = function (scroll) {
    return scroll.slice(4).split('').map((x) => SLANG_NAMES[x]).join(' | ');
}

const generateScrollBlock = function (array) {
    let result = new String();
    for (scroll of array) {
        result += `<div class="combination-box" data-id="${scroll.slice(0, 3)}">
    <div>
        <img src="./assets/images/${WEAPON_ICONS[scroll[0]]}.png">
        <img src="./assets/images/${WEAPON_ICONS[scroll[1]]}.png">
        <img src="./assets/images/${WEAPON_ICONS[scroll[2]]}.png">
    </div>
    <div class="scroll-type">${scrollTypeList(scroll)}</div>
</div>\n`
    }
    return result;
}
...
const limitSkillOptions = function (event) {
    let val = $(this).val();
    if (val != '-1' && val != '0')
        $('select[id^="scroll-skill-"').not(this).each(function () {
            if ($(this).val() == val) {
                $(this).val("-1");
            }
        });
...
const craftScroll = function (searchAny = false) {
    let cArray = [];
    $('select[id^="scroll-component-"').each(function () {
        if ($(this).val() != '-1') {    
    ...
    if (searchAny == true) {
        let results = String();
        SCROLL_COMBINATIONS.forEach(function (scroll) {
            const weapons = scroll.slice(0, 3);
            for (const component of components) {
                if (weapons.search(component) == -1) {
                    return;
                }
            }
            result += `
<div class="combination-box" data-id="${scroll.slice(0, 3)}">
    ...
    <div class="scroll-type">${scrollTypeList(scroll)}</div>
        ...
        $(".combination-box").on("click", clickCombinationBox);
        $('.results-parent').show();
        $("#results-warning").hide();
    }
}

const possibleScrolls = function () {

}

const limitWeaponCombinations = function (event) {
    const val = $(this).val();
    $('select[id^="scroll-component-"').not(this).each(function () {
        if ($(this).val() == val) {
            $(this).val("-1");
        }
    });
